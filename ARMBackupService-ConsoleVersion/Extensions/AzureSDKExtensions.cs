...
namespace ARMBackupService_ConsoleVersion.Extensions
{
	public static class AzureSDKExtensions
	{
		...
		public static async Task<string> ExportKeyVaultSecrets(ResourceManagementClient resourceManagementClient, string subscriptionId, string resourceGroupName, string keyVaultName, string secretName)
		{
			var baseURL = $"{keyVaultName}/secrets/{secretName}?api-version=7.3";
			var _result = await customGET(resourceManagementClient, baseURL);
			return "";
		}

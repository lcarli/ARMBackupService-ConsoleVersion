...
namespace ARMBackupService_ConsoleVersion.Helpers
{
   ...
   public static class StringExtensions
   {
      ...
      public static string FixAKV(this string template, ARMFixedObject fixedObject, ResourceManagementClient restClient, ISubscription subscription, ResourceGroupInner resourceGroup)
      {
         ...
         foreach (var item in resources)
         {
            ...
            if (type == "Microsoft.KeyVault/vaults/secrets")
            {
               ...
               fixedObject.readme.Append($" - Ajouter le valeur pour le secret {secretName.Substring(index + 1).Replace("')]", "")}");
               fixedObject.readme.Append(Environment.NewLine);
